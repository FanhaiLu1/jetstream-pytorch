...
class PageAttentnioTest(parameterized.TestCase):
  ...
  def test_page_attention_update(self):
    ...
    def _insert_prefill(seq_len, dim, slot):
      ...
      k, v = jnp.reshape(k, (1, 1, seq_len, dim)), jnp.reshape(k, (1, 1, seq_len, dim))
      prefill_chache.update(k, v)
      prefill_caches = [prefill_chache]
      prefill_caches = [c.state() for c in prefill_caches]

      return pam.insert_prefill_cache(prefill_caches, decode_caches, slot, seq_len, env.cache_sharding) 
    ...
    xk, xv = jnp.arange(-1, -11, -1).reshape(5, 1, 1, 2), jnp.arange(-1, -11, -1).reshape(5, 1, 1, 2)
