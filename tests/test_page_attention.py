...
class PageAttentnioTest(parameterized.TestCase):
  ...
  def test_prefill_insert(self):
    ...
    pam.insert_prefill_cache(prefill_caches, decode_caches, 1, 3, env.x_sharding)
  ...
  def test_prefill_insert_multiple_pages(self):
    ...
    decode_caches = pam.insert_prefill_cache(prefill_caches, decode_caches, 1, 6, env.cache_sharding) 
  ...
  def test_reserve_pages_decode(self):
    ...
    self.assertTrue(jnp.array_equal(slot_page_indices, expected_slot_page_indices))
    
    lens = jnp.asarray([0, seq_len, 0])
    pam.fill_new_pages(lens)
    expected_slot_page_indices = jnp.asarray([0, 1, 2])
    slot_page_indices = pam.page_indices[slot][0: 3]
    self.assertTrue(jnp.array_equal(slot_page_indices, expected_slot_page_indices))
    
    expected_0_page_indices = jnp.asarray([-1, -1, -1, -1])
  ...
  def test_get_page_token_indices(self):
    ...
    pam = PageAttentionManager(batch_size=5, total_num_pages=20, page_size=4, max_pages_per_sequence=4)
    ...
    self.assertTrue(jnp.array_equal(page_token_indices[0], expected_page_indices))
    self.assertTrue(jnp.array_equal(page_token_indices[1], expected_token_indices))
