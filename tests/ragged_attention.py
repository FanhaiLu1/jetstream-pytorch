...
def partition_by_axis(axis=None):
...
dtype=jnp.float32
bsz, num_heads, seqlen, head_dim = 8, 32, 1, 128
num_kv_heads = 32
cache_len = 16
block_size = 4
k1, k2, k3, k4 = jax.random.split(jax.random.PRNGKey(0), 4)

q = jax.random.normal(k1, (bsz, num_heads, seqlen, head_dim), dtype)
k = jax.random.normal(k2, (bsz, num_kv_heads, cache_len, head_dim), dtype)
v = jax.random.normal(k2, (bsz, num_kv_heads, cache_len, head_dim), dtype)
start = jnp.asarray([11, 0, 10])
input_pos = jnp.asarray([15, 9, 8])
...
env = Env(cache_len, block_size, mesh)

shard_axis = 1
qkv_pspec = partition_by_axis(shard_axis)  # Number of heads
others_pspec = partition_by_axis()
ragged_attention = RaggedAttentionKernel(
    env,
    input_specs=(*([qkv_pspec] * 3), *([others_pspec] * 4)),
    output_specs=(qkv_pspec, (others_pspec, others_pspec)),
    sharding_axis=shard_axis,
)


decode_state = DecodeState(start, input_pos)

ragged_batch_index, ragged_block_index=precompute_ragged_block_indices(env, decode_state)
ragged_batch_index, ragged_block_index = ragged_batch_index.reshape(
    (-1)
), ragged_block_index.reshape((-1))

out, (m, l) = ragged_attention(q, k, v, start, end, ragged_batch_index, ragged_block_index)