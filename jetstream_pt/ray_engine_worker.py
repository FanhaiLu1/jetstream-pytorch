...
class PyTorchEngineRayWorker():
  ...
  def __init__(
    ...
    self._call_model_prefill = jax.jit(self._call_model_prefill, donate_argnums=(0, 1), out_shardings=(self.replicated, self.cache_sharding))
    ...
    self._call_model_generate = jax.jit(self._call_model_generate, donate_argnums=(0, 2, 3, 4, 5, 6, 7), out_shardings=(
        self.replicated,
        self.cache_sharding,
        self.replicated,
        self.replicated,
  ...
  def _load_from_safetensors(self, path):
    ...
    with safe_open(path, framework='flax', device='cpu') as f:
      for key, model_weights in self.pt_model.state_dict().items():
        if key == 'freqs_cis': 
          continue
        v = f.get_tensor(key)
        arr = jax.make_array_from_callback(v.shape, self.sharding_by_name(key), lambda idx: v[idx])
    ...
    weights['freqs_cis'] = jax.make_array_from_callback(freqs_cis.shape, self.replicated, lambda idx: freqs_cis[idx])
    # weights['freqs_cis'] = jax.device_put(freqs_cis, )
...
# def create_pytorch_engine_ray_worker(
#     devices: list[Any],
#     tokenizer_path: str,
#     ckpt_path: Optional[str] = None,
#     samples_per_slot: int = 1,
