...
class PageAttentionManager:
  ...
  def get_page_token_indices(self, lens: jax.Array)-> jax.Array:
    ...
    assert lens.shape == (self.batch_size, 1), f"len shape: {lens.shape} not equals batch size: {self.batch_size, 1}"
    update_page_indices = []
    token_scale_indices = []
    batch_slots = []
    new_lens = []
    ...
    return jnp.stack((jnp.asarray(update_page_indices), jnp.asarray(token_scale_indices), jnp.asarray(batch_slots), jnp.asarray(new_lens)))
